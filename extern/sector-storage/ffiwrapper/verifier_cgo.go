...
func (sb *Sealer) GenerateWinningPoSt(ctx context.Context, minerID abi.ActorID, sectorInfo []proof5.SectorInfo, randomness abi.PoStRandomness) ([]proof5.PoStProof, error) {
	...
	err := sb.sectors.MakeSureSectorsStore(ctx, abi.SectorID{Miner: minerID, Number: sectorInfo[0].SectorNumber})
	if err != nil {
		return nil, xerrors.Errorf("MakeSureSectorsStore failed: %+v", err)
	}
