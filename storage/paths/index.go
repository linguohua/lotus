...
func (i *MemIndex) StorageAttach(ctx context.Context, si storiface.StorageInfo, st fsutil.FsStat) error {
	...
	if _, ok := i.stores[si.ID]; ok {
		...
		s.info.Groups = si.Groups
		s.info.AllowTo = si.AllowTo
		s.info.AllowTypes = allow
		s.info.DenyTypes = deny
		s.info.AllowMiners = si.AllowMiners
		s.info.DenyMiners = si.DenyMiners
...
func (i *MemIndex) allocStorageForFinalize(sector abi.SectorID, ft storiface.SectorFileType) ([]storiface.StorageInfo, error) {
	...
	for j, fileType := range storiface.PathTypes {
		...
		if exist {
			for _, d := range dd {
				...
				if exist {
					// lgh: if sector already present, just ignore storage property
					// if !s.info.CanStore {
					// 	continue
					// }
...
func (i *MemIndex) TryBindSector2SealStorage(ctx context.Context, fileType storiface.SectorFileType, pathType storiface.PathType,
...
func (i *MemIndex) allocStorageForSealing(sector abi.SectorID, groupID string) ([]storiface.StorageInfo, error) {
	var candidates []*storageEntry
	for _, p := range i.stores {
		// only bind to sealing storage
		if !p.info.CanSeal {
...
func (i *MemIndex) UnBindSector2SealStorage(ctx context.Context, sector abi.SectorID) error {
