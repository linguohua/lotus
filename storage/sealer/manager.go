...
func New(ctx context.Context, lstor *paths.Local, stor paths.Store, ls paths.LocalStorage, si paths.SectorIndex, sc config.SealerConfig, pc config.ProvingConfig, wss WorkerStateStore, mss ManagerStateStore) (*Manager, error) {
	...
	queryWorker := true
	if os.Getenv("SCH_QUERY_WORKER") == "false" {
		queryWorker = false
	}
...
func (m *Manager) AddWorker(ctx context.Context, w Worker, url string) error {
	...
	whnd, err := newWorkerHandle(ctx, w, url)
	if err != nil {
		return err
	}
...
func (m *Manager) UpdateFinalizeTicketsParams(ctx context.Context, tickets uint, interval uint) error {
	return m.sched.updateFinalizeTicketsParams(ctx, tickets, interval)
}

func (m *Manager) UpdateP1TicketsParams(ctx context.Context, tickets uint, interval uint) error {
	return m.sched.updateP1TicketsParams(ctx, tickets, interval)
}
...
func (m *Manager) SectorsUnsealPiece(ctx context.Context, sector storiface.SectorRef, offset storiface.UnpaddedByteIndex, size abi.UnpaddedPieceSize, ticket abi.SealRandomness, unsealed *cid.Cid) error {
	...
	// one of it's sealing scratch spaces to store them after fetching them from another worker.
	selector := newExistingSelector(m.queryWorker, m.index, sector.ID, storiface.FTSealed|storiface.FTCache, sector.ID.Miner, "")
...
func (m *Manager) AddPiece(ctx context.Context, sector storiface.SectorRef, existingPieces []abi.UnpaddedPieceSize, sz abi.UnpaddedPieceSize, r io.Reader) (abi.PieceInfo, error) {
	...
	if err == nil {
		groupID, _ := findSectorGroup(ctx, m.index, sector.ProofType, sector.ID, storiface.FTUnsealed)
		if groupID != "" {
			// ugly, for pass groupID to event
			err = fmt.Errorf("ugly:%s", groupID)
		}
	}
...
// lingh: get work and schedule to worker to do
func (m *Manager) SealPreCommit1(ctx context.Context, sector storiface.SectorRef, ticket abi.SealRandomness, pieces []abi.PieceInfo) (out storiface.PreCommit1Out, err error) {
	...
	selector := newExistingSelector(m.queryWorker, m.index, sector.ID, storiface.FTUnsealed|storiface.FTCache|storiface.FTSealed, sector.ID.Miner, groupID)
...
func (m *Manager) SealPreCommit2(ctx context.Context, sector storiface.SectorRef, phase1Out storiface.PreCommit1Out) (out storiface.SectorCids, err error) {
	...
	// lingh: not allow fetch
	selector := newExistingSelector(m.queryWorker, m.index, sector.ID, storiface.FTCache|storiface.FTSealed, sector.ID.Miner, groupID)
...
func (m *Manager) FinalizeReplicaUpdate(ctx context.Context, sector storiface.SectorRef, keepUnsealed []storiface.Range) error {
	...
	selector := newExistingSelector(m.queryWorker, m.index, sector.ID, storiface.FTCache|storiface.FTUpdateCache, sector.ID.Miner, groupID)
...
func (m *Manager) GenerateSectorKeyFromData(ctx context.Context, sector storiface.SectorRef, commD cid.Cid) error {
	...
	// with direct access to the data. We want to do that because this step is
	// generally very cheap / fast, and transferring data is not worth the effort
	selector := newExistingSelector(m.queryWorker, m.index, sector.ID, storiface.FTUnsealed|storiface.FTUpdate|storiface.FTUpdateCache|storiface.FTCache, sector.ID.Miner, groupID)
...
func (m *Manager) ProveReplicaUpdate1(ctx context.Context, sector storiface.SectorRef, sectorKey, newSealed, newUnsealed cid.Cid) (out storiface.ReplicaVanillaProofs, err error) {
	...
	// with direct access to the data. We want to do that because this step is
	// generally very cheap / fast, and transferring data is not worth the effort
	selector := newExistingSelector(m.queryWorker, m.index, sector.ID, storiface.FTUpdate|storiface.FTUpdateCache, sector.ID.Miner, groupID)
